...
class DAN(ModelBase):
    ...
    def __init__(self, embedding_layer, configs):
        ...
        #self.w = nn.Parameter(torch.Tensor(torch.randn(self.n_e, self.n_e)).cuda(), requires_grad=True)
    ...
    def forward_one_side(self, batch):
        ...
        #result = torch.bmm(emb, self.w.unsqueeze(0).expand(emb.size(0), self.w.size(0), self.w.size(1)))
        #emb = F.tanh(result)
    ...
    def __repr__(self):
        text = "DAN (\n{}\n{}\n{}\n)".format(
            ...
            indent(str(self.dropout_op), 2),
            indent(str(self.seq), 2),
            indent(str(self.output_op), 2)
        )
        return text
